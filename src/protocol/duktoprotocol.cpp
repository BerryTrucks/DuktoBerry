...
void DuktoProtocol::newIncomingConnection()
{
    ...
    // Se sto gi� ricevendo o inviando, rifiuto la connessione
            ...
            case FILENAME:
        {
            ...
            if (mElementSize == -1)
            {
                ...
                if (mRootFolderName != rootName) {
                    ...
                    // Verifico se ho gi� una cartella con questo nome
                ...
                // Se invece l'ho gi� trattata, allora rinomino questo percorso
                ...
                QDir dir(".");
                        ...
                        qDebug()<< "DuktoProtocol::readNewData:" << dir.mkpath(name);
                        if (!ret)
                        {
            ...
            {
                // Se il file � in una cartella rinominata, devo provvedere di conseguenza
...
void DuktoProtocol::sendFile(QString ipDest, qint16 port, QStringList files)
{
    ...
    // Verifica altre attivit� in corso
...
void DuktoProtocol::sendData(qint64 b)
{
    ...
    // Se il file corrente non � ancora terminato
